...
def export_seed(request):
    ...
    if user_profile.role == "Field Officer":
        farmers = SeedDistri.objects.filter(field_officer=user_profile).order_by('-issued_on')

    elif user_profile.role == "supervisor":
        supervised_sub_counties = user_profile.sub_county.all()
        farmers = SeedDistri.objects.filter(sub_county__in=supervised_sub_counties).order_by('-issued_on')

    elif user_profile.role == "Project Manager":
        farmers = SeedDistri.objects.all().order_by('-issued_on')
